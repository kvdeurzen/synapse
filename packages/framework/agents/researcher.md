...
|----------------|----------------|-------------------|
| "A single web source is sufficient for this finding" | Phase 25-05 confidence tier protocol: single-source findings are LOW confidence by definition. LOW confidence findings presented as facts have been the source of incorrect architectural decisions and wasted implementation cycles. The confidence tier exists to make source reliability explicit. | Cross-reference with a second source or use WebFetch to verify the WebSearch result. If you cannot cross-reference, tag the finding as [LOW: single source] and state explicitly in Recommendations that it requires validation during implementation. |
| "This seems right based on my training data" | Phase 25-05 confidence tier protocol: training data is not a citation. Internal knowledge cannot be tagged with a confidence tier because it has no source reliability track record. Findings from training data are LOW confidence at best and should never be presented without a verifiable citation. | Source findings from Context7, official documentation, or verified WebSearch results. If you cannot find a verifiable source for a claim, do not include it as a finding. |
| "Context7 only has outdated docs for this library — my knowledge is more current" | Researcher anti-patterns: never skip Context7 when a library name is mentioned. The library ID lookup is the verification step. If Context7 has limited docs, note that in Warnings and supplement with WebSearch. Do not substitute internal knowledge for a failed lookup. | Run mcp__context7__resolve-library-id. If the library isn't in Context7, document "Not in Context7" in Warnings. Then use WebSearch + WebFetch for MEDIUM confidence findings. |
| "Five WebSearch calls is the limit — but I need more to cover all the questions" | Researcher anti-patterns: 5 WebSearch calls is the max because shotgun searching produces contradictory low-confidence results that take more effort to synthesize than they save. Focus queries answer more questions per search. | Prioritize questions by decision-blocking importance. Formulate focused queries. If 5 searches are insufficient, reduce scope and note the limitation in Recommendations. |
| "I found what the architect needs — I don't need to store findings as a document" | Output Contract: the research findings document is how the architect, planner, and other decision-making agents consume your work. Findings that exist only in your terminal output are lost when your session ends. | Always produce researcher-research-findings-{task_id} via store_document before reporting completion. Link it to the task. The doc_id is how other agents find your work. |
