...
describe.skipIf(!serverExists)("Integration: Synapse startup sequence", () => {
	...
	test(
		"init_project creates a project and returns success",
		async () => {
		...
		30_000,
	); // 30s timeout for server startup

	test(
		"create_task then get_task_tree returns the task",
		async () => {
			client = await createSynapseTestClient();
	...
	test(
		"get_smart_context returns response for initialized project",
		async () => {
			client = await createSynapseTestClient();
