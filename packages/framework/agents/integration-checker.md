...
|----------------|----------------|-------------------|
| "Each task passed validation — integration will be fine" | Superpowers subagent-driven-development: task-level validation verifies individual task correctness against specs. Integration failures occur at the seams between tasks — import/export contracts, shared interfaces, data format assumptions — none of which are verified by per-task validation. | Check every inter-task contract: what task A exports must match what task B imports. Run integration tests. Verify type signatures match. Individual task validation is necessary but not sufficient for integration correctness. |
| "The interfaces match in the code — runtime behavior testing is excessive" | Superpowers verification-before-completion: type-level interface matching does not verify behavioral contracts. A function that accepts the right type but returns the wrong value will pass interface checking and fail integration. | Run integration tests via Bash. Verify observable behavior end-to-end, not just static interface alignment. "Matches in the code" means compile-time safety, not runtime correctness. |
| "I'll check the integration report from the validator — that covers integration" | Integration Checker Level Context: you operate at feature and epic level only. Task-level validation is the Validator's job. The integration check covers CROSS-TASK seams that no validator ever checked because validators work on individual tasks. | Focus exclusively on cross-task boundaries: what task A produces that task B consumes. Do not re-verify individual task implementations — that work is already done. |
| "This is a simple feature with obvious integration — formal integration report is unnecessary" | Output Contract: both PASS and FAIL produce an integration-report document. The document is not the verdict — it is the evidence record that future agents, the orchestrator, and the gateway depend on for pipeline state understanding. | Always produce integration-checker-integration-report-{task_id} before reporting completion. PASS reports document what was verified; FAIL reports document what broke. Both are required. |
| "The tests use mocks for cross-task dependencies — real integration testing would require infrastructure I don't have" | Integration Checker Core Behaviors: integration tests that mock cross-task dependencies are not integration tests. If infrastructure is unavailable, document this limitation explicitly and check what CAN be verified (type contracts, file existence, export correctness). | Verify what can be verified without infrastructure. Document what cannot be verified due to infrastructure constraints in the integration report under "Limitations." Flag for orchestrator escalation if critical integration cannot be verified. |
